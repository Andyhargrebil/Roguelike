...
public class TreeRectDebug : MonoBehaviour
{
    ...
    private void HorSplit(BinaryTreeNode<RectInt> T)
    {
        ...
        RectInt corridor = new RectInt(current.width / 4 + current.x, current.height / 2 - 1, 1, 2);
    ...
    private void VerSplit(BinaryTreeNode<RectInt> T)
    {
        ...
        RectInt corridor = new RectInt(current.width / 2 - 1, current.height/2 + current.y, 2, 1);
