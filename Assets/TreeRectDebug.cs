...
public class TreeRectDebug : MonoBehaviour
{
  ...
  private void HorSplit(BinaryTreeNode<RectInt> T)
  {
    ...
    RectInt corridor = new RectInt(upperRect.width / 2 + current.x, current.height / 2 - 1, 1, 2);
    MakeCorridor(corridor, "V");
  }
  ...
  private void VerSplit(BinaryTreeNode<RectInt> T)
  {
    ...
    RectInt corridor = new RectInt(current.width / 2 - 1, leftRect.height / 4 + current.y, 2, 1);
    MakeCorridor(corridor, "H");
  }
